...
import { getPublicEventByIdentifier, listPublicMatches, listPublicStandings, listPublicTeams } from '@/lib/db/repositories/public-scores'
...
export default async function PublicLivePage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const event = await getPublicEventByIdentifier(eventId)
  if (!event) notFound()
  const [matches, standings, teams] = await Promise.all([listPublicMatches(event.id), listPublicStandings(event.id), listPublicTeams(event.id)])
