  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-6 bg-background px-6 text-foreground">
      ...
      <nav aria-label="Workspace navigation" className="flex flex-wrap justify-center gap-3">
        {user.role === 'SUPER_ADMIN' && <Link href="/super-admin/dashboard" className="rounded-md border border-border px-4 py-2 text-sm font-medium hover:bg-muted">Open admin</Link>}
        {user.role === 'EVENT_ADMIN' && <Link href="/event-admin/dashboard" className="rounded-md border border-border px-4 py-2 text-sm font-medium hover:bg-muted">Open event workspace</Link>}
