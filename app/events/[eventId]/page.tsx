...
export default async function EventResourcesPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  return <main className="min-h-screen bg-background px-4 py-8 text-foreground sm:px-6">
    <div className="mx-auto flex max-w-6xl flex-col gap-8">
      <header className="flex flex-wrap items-end justify-between gap-4"><div className="flex flex-wrap gap-3"><Link href={`/events/${eventId}/matches`} className="rounded-md border border-border px-3 py-2 text-sm font-medium hover:bg-muted">Matches & schedule</Link><Link href={`/events/${eventId}/scorers`} className="rounded-md border border-border px-3 py-2 text-sm font-medium hover:bg-muted">Event team</Link></div>
