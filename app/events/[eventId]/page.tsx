...
export default async function EventResourcesPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const user = await requireEventAdmin(eventId)
  ...
  return <main className="min-h-screen bg-background px-4 py-8 text-foreground sm:px-6">
    <div className="mx-auto flex max-w-6xl flex-col gap-8">
      <header className="flex flex-wrap items-end justify-between gap-4"><div className="flex flex-wrap gap-2"><Link href={`/events/${eventId}`} className="rounded-md bg-muted px-3 py-2 text-sm font-medium">Overview</Link><Link href={`/events/${eventId}/matches`} className="rounded-md border border-border px-3 py-2 text-sm font-medium hover:bg-muted">Matches & schedule</Link><Link href={`/events/${eventId}/scorers`} className="rounded-md border border-border px-3 py-2 text-sm font-medium hover:bg-muted">Scorers</Link></div>
        <div className="flex flex-col gap-2"><Link href={user.role === 'SUPER_ADMIN' ? '/admin' : '/event-admin'} className="text-sm text-muted-foreground hover:text-foreground">← {user.role === 'SUPER_ADMIN' ? 'Admin' : 'Event Operations'}</Link><p className="text-sm font-semibold uppercase tracking-[0.2em] text-muted-foreground">Event configuration</p><h1 className="text-3xl font-semibold tracking-tight">{event.name}</h1><p className="text-muted-foreground">Configure the sports, teams, players, and courts for this event.</p></div>
      </header>
      <section className="rounded-xl border border-border bg-card p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Step 1</p><h2 className="mt-1 text-lg font-semibold">Choose sports</h2><p className="mt-1 text-sm text-muted-foreground">Select the disciplines this event will run. Selected sports stay attached to its schedule.</p></div><span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary">{selected.length} selected</span></div>{allSports.length ? <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{allSports.map((sport) => { const enabled = selectedIds.has(sport.id); return <form action={async () => { 'use server'; await toggleEventSport(eventId, sport.id, !enabled) }} key={sport.id} className={`flex items-center justify-between gap-3 rounded-lg border p-4 transition-colors ${enabled ? 'border-primary/50 bg-primary/10' : 'border-border bg-background/40 hover:border-primary/30'}`}><div><p className="font-medium">{sport.name}</p><p className="text-xs text-muted-foreground">{enabled ? 'Included in this event' : 'Available from catalog'}</p></div><button className={`rounded-md px-3 py-1.5 text-sm font-medium ${enabled ? 'bg-primary text-primary-foreground' : 'border border-border hover:bg-muted'}`} type="submit">{enabled ? 'Selected' : 'Add'}</button></form>})}</div> : <div className="mt-5 rounded-lg border border-dashed border-border p-8 text-center"><p className="font-medium">No sports in the catalog</p><p className="mt-1 text-sm text-muted-foreground">Add sports to the global catalog before configuring this event.</p></div>}</section>
