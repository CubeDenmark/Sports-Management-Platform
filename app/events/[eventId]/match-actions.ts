...
export async function listAssignedMatches() {
  ...
  return db.select({ match: matches, eventName: events.name }).from(matchScorers).innerJoin(matches, eq(matches.id, matchScorers.matchId)).innerJoin(events, eq(events.id, matches.eventId)).innerJoin(users, eq(users.id, matchScorers.userId)).where(and(eq(matchScorers.userId, user.id), eq(matchScorers.status, 'ACTIVE'), eq(users.role, 'SCORER'), eq(users.isActive, true))).orderBy(matches.scheduledStart)
}
