...
export function AdminShell({ children, user, events = [], eventId: _eventId }: { children: React.ReactNode; user: { displayName: string; role: string }; events?: EventOption[]; eventId?: string }) {
  ...
  const goToEvent = (eventId: string) => {
    ...
    router.push(`/admin/events/${eventId}/overview`)
  }
  ...
  return <div className="min-h-screen bg-background text-foreground lg:flex">
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-sidebar-border bg-sidebar px-4 py-5 transition-transform lg:static lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <nav className="mt-4 flex flex-1 flex-col">{user.role === 'SUPER_ADMIN' ? <><NavGroup label="Platform" items={superAdminNavigation} pathname={pathname}/><NavGroup label="System" items={system} pathname={pathname}/></> : user.role === 'EVENT_ADMIN' ? <><div className="flex flex-col gap-2 px-1 pt-6"><label htmlFor="event-selector" className="px-2 text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">Event</label><select id="event-selector" value={selectedEventId} disabled={events.length === 0} onChange={(event) => goToEvent(event.target.value)} className="w-full rounded-lg border border-sidebar-border bg-sidebar-accent px-3 py-2.5 text-sm text-sidebar-foreground outline-none focus:ring-2 focus:ring-sidebar-ring"><option value="">{events.length === 0 ? 'No assigned events' : 'Select an event'}</option>{events.map((event) => <option key={event.id} value={event.id}>{event.name}</option>)}</select>{selectedEvent && <p className="truncate px-2 text-xs text-sidebar-foreground/60">{selectedEvent.name}</p>}</div>{selectedEvent ? <NavGroup label="Event operations" items={eventAdminNavigation(selectedEvent.id)} pathname={pathname}/> : <p className="px-3 pt-6 text-sm text-sidebar-foreground/60">Ask a Super Admin to assign an event.</p>}</> : <NavGroup label="Scorer" items={scorerNavigation} pathname={pathname}/>}</nav>
